            ...
            <div class="space-y-3">
                <form method="POST" action="{{ route('settings.update') }}" class="space-y-3">
                    ...
                    @foreach ($settings as $setting)
                        <div class="rounded-2xl border border-white/5 bg-white/5 p-4 flex items-start justify-between gap-4">
                            ...
                            <label class="relative inline-flex h-6 w-11 cursor-pointer items-center">
                                <input type="checkbox" name="settings[{{ $loop->index }}][enabled]" value="1" class="peer sr-only" {{ $setting->enabled ? 'checked' : '' }}>
                    ...
                    <div class="flex justify-end">
                        <button class="rounded-xl bg-gradient-to-r from-sky-500 to-emerald-500 px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-sky-500/20">
                            Save toggles
                        </button>
                    </div>
                </form>
